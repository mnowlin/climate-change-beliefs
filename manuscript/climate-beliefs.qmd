...
<!-- FLAGGED, NOT EDITED (R1-5): R1 found this section's literature review "comprehensive but could be more focused, particularly in integrating hierarchical and network perspectives rather than presenting them in parallel" -- i.e., the section below largely covers the hierarchical approach, then the network approach, as two separate traditions rather than weaving them together. This is a structural/rhetorical restructuring call across ~50 lines of established prose, not a targeted language fix, so I did not attempt it -- doing so risked changing your argument's flow and content in ways that need your own judgment. Worth a close read of this section with that critique in mind. -->
...
<!-- NEEDS YOUR DETERMINATION (R1-6 / R2 hypothesis precision): the two sentences above map each theoretical account onto specific centrality measures (hierarchical -> betweenness/closeness; solution aversion -> strength). This is a genuine interpretive call, not a settled fact -- please confirm you agree with this mapping, or reframe it. Also note the gateway belief model isn't mapped to a specific measure here, since it's fundamentally a claim about causal direction rather than structural position; whether that gap needs addressing explicitly (vs. leaving it to the Discussion's GBM reframing) is your call too. -->
...
It is worth noting that this operationalization of perceived scientific consensus differs from the framing used in much of the gateway belief model literature, which typically presents respondents with an explicit numerical consensus estimate (e.g., that 97 percent of climate scientists agree that climate change is happening) before asking about downstream beliefs [@vanderlindenGatewayBeliefModel2019]. The sciconsensus measure used here instead asks respondents for their own naturalistic belief about whether such a consensus exists, without providing a specific consensus figure. This is a more conservative measure in the sense that it does not prime respondents with the consensus estimate itself, so it may better capture organically held beliefs about scientific consensus rather than beliefs induced by exposure to a consensus statistic.

<!-- NEEDS YOUR DETERMINATION (R2-3): this paragraph acknowledges the sciconsensus operationalization difference R2 raised. I framed it as a methodological distinction (naturalistic vs. primed measure) rather than apologizing for it as a limitation, but how far to lean into "this is a deliberate contribution" language vs. treating it more cautiously as a limitation is your call -- see reviews/response-letter.md R2-3 for both framing options that were considered. -->
...
<!-- FLAGGED, NOT EDITED (R1-7): R1 found this Methods section "technically sound but could be streamlined for readability." "Streamline" wasn't specific enough for me to safely act on unilaterally -- cutting content risks removing something you want, and I don't have a clear target for what to trim vs. keep. Worth your own read-through with an eye toward tightening; I did not restructure or cut anything here. -->
...
<!-- NEEDS YOUR DETERMINATION (R1-4): this new subsection covers the transparency items R1 asked for (preregistration statement, power-analysis substitute, robustness checks pointer -- the checks themselves are addressed elsewhere). What it does NOT specify is how a reader would actually access the data/code -- that's your call: (a) make the repository public and add its URL here, (b) state data/code are available from the author upon request, or (c) something else. Please fill that in before submission. -->
...
Second, I re-estimated the network across a range of EBIC tuning values (gamma = 0, 0.25, 0.5, 0.75, 1.0) to check whether the reported results depend on the tuning = 0.5 default used above. The number of non-zero edges ranges from `r min(ggm_robustness$robust_df$n_edges)` to `r max(ggm_robustness$robust_df$n_edges)` across this range, but the node with the highest strength, betweenness, and closeness centrality is unchanged at every value tested.

Third, I compared the EBIC-selected network to one selected via cross-validation, using the `huge` R package. The cross-validated network has `r ggm_robustness$n_edges_cv` edges, compared to `r ggm_robustness$n_edges` from the EBIC-selected network -- a materially similar level of sparsity.

Finally, I conducted pairwise bootstrap difference tests for all `r ggm_robustness$diff_between_n_pairs` possible node pairs on both betweenness and strength [See @epskampBootnetBootstrapMethods2023]. `r ggm_robustness$diff_between_n_sig` of `r ggm_robustness$diff_between_n_pairs` pairwise comparisons were statistically significant for betweenness, and `r ggm_robustness$diff_strength_n_sig` of `r ggm_robustness$diff_strength_n_pairs` were significant for strength. Among the five nodes with the highest betweenness centrality, none of the `r ggm_robustness$top5_between_n_pairs` pairwise comparisons reached significance, meaning their betweenness centrality is not statistically distinguishable from one another. Among the five nodes with the highest strength centrality, only `r ggm_robustness$top5_strength_n_sig` of `r ggm_robustness$top5_strength_n_pairs` pairwise comparisons was significant.

<!-- NEEDS YOUR DETERMINATION (R2-4, R2-5): the paragraph above reports the pairwise difference-test results factually, but I did not add interpretive language about what this means for how confidently the "most central" claims elsewhere in the paper should be stated -- e.g., the betweenness result means egalitarianism's point-estimate rank as most central is not statistically distinguishable from the next several nodes. Whether and how strongly to soften rank claims elsewhere in Results/Discussion in light of this is your call; I did not do it here since it touches claims throughout the paper, not just this section. -->
...
<!-- NEEDS YOUR DETERMINATION (R1-8): this paragraph is an attempt to more explicitly state the paper's contribution to policy process research and climate communication debates, per R1's request for a clearer articulation of contribution. The claims synthesize points already made elsewhere in the paper, but how strongly to state the contribution -- and whether this is the right emphasis -- is your call; please review closely rather than take this as settled. -->
